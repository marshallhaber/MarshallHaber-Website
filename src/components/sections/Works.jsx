...
export default function Works() {
  ...
  return (
    <div ref={containerRef} className="w-full px-6 pt-12 pb-10 bg-[#fbf0f2] text-[#020817]">
      <Reveal>
        <h2
          className="text-[clamp(1.5rem,4vw,3.2rem)] font-bold leading-[1.1] mb-16 text-inherit text-center md:whitespace-nowrap"
        ...
        {featured.map((p, i) => {
          ...
          return (
            <TransitionLink to={`/work/${p.slug}`} key={p.slug} className="block">
              ...
              >
                <div className="w-full aspect-[4/3] rounded-2xl overflow-hidden relative mb-6 bg-white">
                  {p.video ? (
                    <video
                      src={p.video}
                      autoPlay
                    ...
                    <img
                      ...
                      loading="lazy"
                      decoding="async"
                      className={`absolute inset-0 w-full h-full ${/crossriver|_BIG/i.test(p.image || '') ? 'object-contain p-6' : 'object-cover'} transition-transform duration-700 group-hover:scale-105`}
                    />
                  )}
                </div>
